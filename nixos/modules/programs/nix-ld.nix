...
in
{
  ...
  options.programs = lib.genAttrs suffixes (
    suffix:
    let
      inherit (variant suffix) name cfg pkgs;
    in
    {
  ...
  config = lib.mkMerge (
    lib.forEach suffixes (
      suffix:
      let
        inherit (variant suffix)
          name
          cfg
          pkgs
          ldso
          ;
      in
      lib.mkIf config.programs.${name}.enable {
        ...
        environment.sessionVariables = {
          "NIX_LD_${
            builtins.replaceStrings [ "-" ] [ "_" ] pkgs.system
          }" = "/run/current-system/sw/share/${name}/lib/ld.so";
          "NIX_LD_LIBRARY_PATH_${
            builtins.replaceStrings [ "-" ] [ "_" ] pkgs.system
          }" = "/run/current-system/sw/share/${name}/lib";
        };

        # We currently take all libraries from systemd and nix as the default.
        # Is there a better list?
        programs.${suffix}.libraries = with pkgs; [
          zlib
          zstd
          stdenv.cc.cc
          curl
