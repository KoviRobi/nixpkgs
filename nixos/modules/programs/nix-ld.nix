...
in
{
  ...
  options.programs = lib.genAttrs suffixes (suffix: let inherit (variant suffix) name cfg pkgs; in {
  ...
  config = lib.mkMerge (lib.forEach suffixes (suffix: let inherit (variant suffix) name cfg pkgs ldso; in lib.mkIf config.programs.${name}.enable {
